...
방법: URL host 기준 분류 (host 27종 → 4 카테고리)

## 분류 기준

| 아이콘 | 의미 | 처리 방향 |
|---|---|---|
| 🔴 | 라이선스 리스크 높음 — 출시 전 폐기 후보 | 자체 일러스트 또는 공식 출처로 교체 |
| 🟡 | 조건부 — 인용 컨텍스트면 가능 / 매체 협의 필요 | 사용 시 ToS 검토 + 출처 표기 |
| 🟢 | 공식·공공 — 사용 가능성 높음 | 각 ToS 별도 확인 후 사용 |
...
- 🔴 분류 203개는 출시 전 폐기 대상. 자체 카테고리 일러스트 폴백 또는 공식 출처 재수집으로 교체.
- 🟡 10개는 사용 시 출처 표기 + ToS 검토 필수.
- 🟢 37개는 각 제조사·기관 ToS 확인 후 사용 가능성 높음.
- 후속 단계 (출시 직전):
